...
const deployedUrl = siteUrlFromEnv ?? "https://poweroninterio.com";
const oldUrl = "https://poweroninterio.com";
const textFileExtensions = new Set([".html", ".txt", ".xml", ".webmanifest"]);

// If no base path, skip.
if (!repoBase) {
  console.log("BASE_PATH not set and no path in NEXT_PUBLIC_SITE_URL; skipping GitHub Pages rewrite.");
  process.exit(0);
}

// Regex to find absolute href/src values; we will guard in code to avoid double-prefix.
const hrefSrcPattern = /(href|src)=([\"'])\/(?!\/)([^\"'>]+)/g;
...
function walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    updated = updated.replace(
      hrefSrcPattern,
      (_match, attr, quote, pathPart) => {
        // Skip if already prefixed with repoBase
        if (pathPart.startsWith(repoBase.slice(1))) {
          return `${attr}=${quote}/${pathPart}`;
        }
        return `${attr}=${quote}${repoBase}/${pathPart}`;
      },
    );
